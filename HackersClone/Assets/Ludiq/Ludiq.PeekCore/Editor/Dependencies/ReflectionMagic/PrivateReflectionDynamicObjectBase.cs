...
namespace Ludiq.PeekCore.ReflectionMagic
{
    public abstract class PrivateReflectionDynamicObjectBase : DynamicObject
    {
        ...
        public override bool TryGetMember(GetMemberBinder binder, out object result)
        {
            ...
            IDynamicAccessor prop = GetProperty(binder.Name);
        ...
        public override bool TryGetIndex(GetIndexBinder binder, object[] indexes, out object result)
        {
            ...
            IDynamicAccessor prop = GetIndexProperty();
        ...
        private IDynamicAccessor GetProperty(string propertyName)
        {
            // Get the list of properties and fields for this type
            IDictionary<string, IDynamicAccessor> typeProperties = GetTypeProperties(TargetType);

            // Look for the one we want
            if (typeProperties.TryGetValue(propertyName, out IDynamicAccessor property))
                return property;
        ...
        private IDictionary<string, IDynamicAccessor> GetTypeProperties(Type type)
        {
            // First, check if we already have it cached
            if (PropertiesOnType.TryGetValue(type, out IDictionary<string, IDynamicAccessor> typeProperties))
                return typeProperties;

            // Not cached, so we need to build it
            typeProperties = new Dictionary<string, IDynamicAccessor>();

            // First, recurse on the base class to add its fields
            if (type.GetTypeInfo().BaseType != null)
            {
                foreach (IDynamicAccessor prop in GetTypeProperties(type.GetTypeInfo().BaseType).Values)
                {
